...
chat REPL — a scrolling list of messages, spinners, an input line. CRIME is a four-pane IDE with a
...
| Terminal emulation | `vt100` | **`wezterm-term` is not published to crates.io** — corrected after trying to add it. `vt100` parses escape sequences into a cell grid and, crucially, exposes `mouse_protocol_mode()`, which is exactly R10.5's question: has the running program asked for mouse events? Its `Callbacks::unhandled_csi` is the other load-bearing capability: every sequence it does not implement — the child's questions among them — is surfaced rather than dropped, so CRIME can answer them without a parser of its own. `alacritty_terminal` and `termwiz` are the alternatives if more fidelity is needed |
...
| Process liveness | `libc` | The Sidecar sweep on start (ADR 0016) deletes every `~/.crime/paths/` directory whose CRIME is gone, and *gone* is `kill(pid, 0)` — no signal delivered, only the question asked. `libc` was already in the tree under `which`, so the crate is free; the alternative was shelling out to `kill -0` per candidate directory, which spawns a process to ask a question the syscall answers and reads `EPERM` as death. Named only in `src/main.rs`, in `alive`, which is the one `unsafe` block in CRIME: `EPERM` counts as alive, because the one mistake this check exists to avoid is deleting the Sidecar of a running instance. **Unix-only, and the first thing in the tree that is**: `kill` has no Windows spelling, so a port needs its own answer here — CRIME already drives a pty and reads `stty`, so this is not where portability is lost |
...
- **`vt100` + `portable-pty`** are what make F3 and R10.5 work: the injected command goes into a
  terminal model CRIME owns, and `mouse_protocol_mode()` answers whether the child wants the click.
...
```
/home/me/projects/crime/src              (bare)
'/home/me/projects/crime/my notes'       (single-quoted)
"/home/me/projects/crime/don't"          (double-quoted)
'/home/me/projects/crime/a;b'            (single-quoted)
```
