...
a program in its own terminal, inside Varde's. It is a hosted pane: its keyboard, mouse and
clipboard belong to the program running in it, and Varde's job is to carry your keys in and its
...
CLI rather than quitting Varde or opening its search. The keys Varde keeps for itself are an
...
the pair Varde counted beside them opens the palette. From the palette, `o` returns you to the
...
The environment the CLI sees is Varde's, not your host terminal's: `TERM` is `xterm-256color`, and
the markers a terminal emulator exports about itself are removed, so a CLI that sniffs for
`TERM_PROGRAM` or `TMUX` gets an answer about Varde rather than about your machine. Asked by
escape sequence, Varde answers as `VARDE(x.y.z)`, declines advanced modifier reporting out loud, and
...
runs on the terminal's alternate screen, where there is no history for Varde to show, so the program
...
`.varde/reviews/NNNN.json` and sends the CLI a prompt — an inline summary of the comments with their
...
is showing, which may be a half-written message Varde cannot read back. With no session running,
...
What does *not* happen: nothing comes back. Varde does not read the CLI's screen to learn whether a
...
Nothing in Varde tests which CLI is running in the pane — not by name, not by version, not by
...
work the same way as the ones that have, and why a newly bound key in your CLI needs no Varde
release.

The consequence for you: if a CLI misbehaves inside Varde, the fault is in the transport, and fixing
...
- **A modifier that does nothing.** On macOS, Option is not Alt unless your terminal is told so
  ...
  Varde owns has a modifier-free route — the palette — but a CLI's own Alt bindings need the terminal
  to send Alt.
- **Clicks leaving stray characters in the prompt.** The CLI asked for one mouse encoding and is
  getting another. That is a Varde transport bug, and the same one for every program.
- **A shortcut that seems disabled.** It is not gated on Varde advertising anything — that was
