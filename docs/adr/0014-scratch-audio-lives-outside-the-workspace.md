...
**It lives in `~/.crime/tmp/`, and it is the first thing CRIME has ever written to the global
directory.**

## Not in the project's `.crime/`

Everything else CRIME writes goes there: `risk.json`, `state.json`, `snapshots/`, `stories/`,
`reviews/`, the seeded `config.toml`. Consistency argues for audio joining them, and consistency is
wrong here for one concrete reason: **the workspace is rendered.** `.crime/` sits in the file tree the
...
The rule underneath is not "audio is special". It is that **`.crime/` holds what belongs to the
...
almost every program gives. It is rejected because CRIME cannot then clean up after itself with any
...
match against somebody else's filenames, on a directory whose contents CRIME has no business
enumerating.

A private directory makes the sweep trivial and safe: **everything in `~/.crime/tmp/` is CRIME's, so
everything in it can go.** That is a rule with no exceptions to get wrong, which is the only kind
worth writing.

It is also where the user already looks. `~/.crime/config.toml` is a path they have opened. A
mystery in `~/.crime/tmp/` is a mystery with an obvious owner; the same file under `/var/folders/…`
belongs to nobody.
...
`~/.crime/config.toml` — about CRIME editing a file a human maintains, clobbering deliberate edits
...
The distinction is worth stating because the shorthand is easy to misremember. **CRIME does not write
the user's configuration. CRIME may write its own scratch.** The global directory is not one file's
folder; it is CRIME's folder, and it happened to contain exactly one read-only file until now.

## Consequences

**The directory is created, not assumed.** `~/.crime/` has only ever been read — `main.rs` opens
...
**Deletion happens twice, deliberately.** When the player exits, and again when CRIME exits. Two
...
**Nothing in the workspace changes when CRIME speaks.** No file appears in the tree, `git status` is
