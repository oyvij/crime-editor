...
The AI pane and the shell pane host a child process, and Varde used to interpret their keys the way
...
A **hosted pane** is now a terminal Varde hosts, not a pane Varde interprets. Its keyboard, mouse
and clipboard belong to the child, and Varde claims an exhaustive, written-down list of **reserved
...
The trade-off is Varde's own bindings inside these two panes, given up on purpose. `Ctrl+Q` does not
...
of keys Varde takes from every CLI forever, and that set only ever grows — a key Varde keeps is a
...
the palette *over* a child that binds `Esc Esc` itself, and from that moment the keys are Varde's.

## What answering the child's queries actually did

A terminal is two-way, and Varde had never answered a child anything. The leading hypothesis behind
...
Claude Code v2.1.240 was driven through a real pty twice — once with Varde's replies active, once
...
Varde answers the queries anyway, and not as a consolation prize: an unanswered question leaves a
program guessing, and a child's behaviour inside Varde is meant to be a fixed target rather than a
...
since asserting a value for a resource Varde has never heard of is not refusing anything. The device
attributes name a deliberately low VT220 so every channel says the same "no": claim a recent xterm
and a program will enable `modifyOtherKeys` and then read Varde's legacy key bytes under the wrong
...
Asked by name (XTVERSION), Varde answers `VARDE(x.y.z)` — the honest answer, and the only one no
...
Both answers are Varde's rather than the host's — `queries::CHILD_ENV` sets `TERM` and `COLORTERM`
...
that sets the mode blind, rather than querying, will read Varde's legacy key bytes under the wrong
...
enforceable. A provider Varde has never been run against works for the same reason the others do,
and a newly bound provider feature needs no Varde release.
...
encoding, or a cell the legacy encoding has no byte for, leaves the wheel scrolling Varde's own
