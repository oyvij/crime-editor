//! What CRIME answers when the child asks its terminal a question — with an
...
/// fixed target rather than a function of which terminal CRIME was launched
/// from, and `TERM` was only the variable most programs read.
///
/// CRIME sets the two it can answer for and removes the rest rather than
/// inventing values. `TERM_PROGRAM` unset is what xterm and Alacritty give a
/// child too, so a program that sniffs it takes its generic path; a name CRIME
/// made up is a name someone will branch on. The version query below does name
/// CRIME, and the difference is what silence means on each channel: an unset
...
/// 24-bit colour, and CRIME passes
...
/// acceptable *here* and nowhere else in CRIME: emulators are a small,
/// slow-moving, well-known set, a missed marker costs a cosmetic difference in
/// one CLI rather than a broken feature, and no CRIME behaviour depends on the
...
pub const CHILD_ENV: &[(&str, Option<&str>)] = &[
    ...
    // finds `TMUX` set will speak tmux's passthrough sequences at CRIME, which
...
/// or nothing when the sequence asked CRIME nothing.
pub fn reply(
    ...
    match (intermediate, final_byte, first) {
        ...
        // What terminal this is, by name and version (XTVERSION). CRIME names
        ...
        (Some(b'>'), 'q', None | Some(0)) => {
            Some(format!("\x1bP>|CRIME({})\x1b\\", env!("CARGO_PKG_VERSION")).into_bytes())
        }
        // A VT220 that speaks ANSI colour: everything CRIME's terminal model
        ...
        // features on this number — modifier reporting among them — and CRIME
        ...
        // point, and asserting a value for a resource CRIME has never heard of is
...
mod tests {
    ...
    #[test]
    fn a_version_query_is_answered_with_crimes_own_name_and_version() {
        for params in [&[&[0u16][..]][..], &[]] {
            let answer = answer(Some(b'>'), params, 'q', (0, 0));
            assert_eq!(
                answer,
                format!("\x1bP>|CRIME({})\x1b\\", env!("CARGO_PKG_VERSION"))
            );
            assert!(answer.starts_with("\x1bP>|CRIME("), "{answer:?}");
    ...
    // reads the attributes answer as "unsupported". That answer — which CRIME now
    // gives — is the refusal. Replying `CSI ? 0 u` would instead claim support
    // for a protocol CRIME does not implement, and the program would then read
    // CRIME's legacy key bytes under kitty's rules.
    ...
    // the point, asserting a value for a resource CRIME has never heard of is not.
    #[test]
    ...
    fn a_sequence_that_is_not_a_query_yields_no_reply() {
        ...
        // of the status one: a parameter CRIME has never heard of gets nothing,
        // as it does for the modifier resources.
        assert!(reply(Some(b'>'), &[&[1]], 'q', (3, 7)).is_none());
        assert!(reply(Some(b'?'), &[&[5]], 'n', (3, 7)).is_none());
    }
    // The identity CRIME hands a child through its environment, the other half
    // of the identity the queries above answer in escape sequences.
    #[test]
    fn the_child_is_told_the_terminal_crime_actually_renders() {
    ...
    fn every_other_variable_is_removed_rather_than_given_a_made_up_value() {
        for (name, value) in CHILD_ENV {
            assert!(
                ...
                "{name} claims a value CRIME cannot answer for"
            );
        }
    }
    ...
    #[test]
    fn no_emulator_the_user_may_have_launched_crime_from_leaves_a_marker() {
        for name in [
            "TERM_PROGRAM",
